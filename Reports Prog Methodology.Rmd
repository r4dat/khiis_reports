...
```
The result is passed or piped to the summarise verb which splits by the cartesian cross-product of all selected grouping vriables E.g. M, Johnson County, 0-19, 1-3, 2010 etc. On each grouping it creates new columns based on the calculation functions
passed.
```{r,eval=FALSE}
...
```
The above lines calculate the median per grouping of total cost "Med", as well as an Average total cost "AvgTot". The ifelse performs the Average Total calculation as previously described and takes the form IF(Test,TRUE,FALSE). The "n()" returns the number of rows/entries in the given grouping. (A number of variable type-casts have been removed for readability.) 
